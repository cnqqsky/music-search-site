...
$(function() {
  ...
  function showToast(message) {
    var $toast = $('.music-toast');
    if (!$toast.length) {
      $toast = $(
        '<div class="music-toast"><span class="music-toast-text"></span></div>'
      ).appendTo('body');
    }
    $toast.find('.music-toast-text').text(message);
    $toast.addClass('is-visible');
    clearTimeout($toast.data('toastTimer'));
    $toast.data(
      'toastTimer',
      setTimeout(function() {
            ...
            success: function success(result) {
              if (result.code === 200 && result.data) {
                ...
                if (page === 1) {
                  ...
                  showToast('对了，祈福提醒您：不能播放的音乐就是版权限制，继续点下一首就行！！！');
